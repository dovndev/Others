...
function imgerror(elm) {
  elm.src = "https://semantic-ui.com/images/wireframe/image.png";
}
...
function GetPreview() {
  ...
  fetch(
    `https://api.linkpreview.net/?key=44ab599c49f52aa82bb84040cf5d3748&q=${input.value}`
  )
    .then((res) => res.json())
    .then((json) => {
      ...
      const html = ` 
       <div class="img-cont">
        <img src="${json.image}" onerror="imgerror(this)">
       </div>
       <div class="details">
      ...
      if (json.error || json.url === "" || notok) {
        alert(
          json.description.includes("exceeded")
            ? json.description
            : "Cannot find any preview for the requested Link"
        );
        NewElment.remove();
      } else {
